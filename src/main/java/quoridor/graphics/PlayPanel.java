...
public class PlayPanel extends JPanel implements MouseListener, MouseMotionListener, ActionListener {
    ...
    public PlayPanel(JFrame jFrame, Color backgroundColor, int size1, int size2, int numberPlayers, int wallDimension, int numberWalls) throws PositionException, NumberOfPlayerException {
        this.jFrame = jFrame;
    ...
    private void setup() throws PositionException, NumberOfPlayerException {
        ...
        gameEngine = new GameEngine(numberPlayers, names, size1, size2, numberWalls, GameType.CONSOLE_GAME, OpponentType.HUMAN);
        ...
        phaseSelectWhatToDo = true;
        phaseMoveMeeple = false;
        phasePlaceWall = false;
    }
    ...
    private void printBoard(Graphics2D g2d){
        ...
        for (int i = board.getRows() - 1; i >= 0; i--) {
            int startX = width/2 - (size1 * (tile.getWidth() + 4))/2;
    ...
    private void printFinalScreen(Graphics2D g2d){
    ...
    public void mouseClicked(MouseEvent e) {
        int x = e.getX();
        int y = e.getY();
        ////////////CLICKS FOR ENTRY/INITIAL PLAYER PHASE////////////////////
        if(phaseSelectWhatToDo) {
            if (rectMoveB.contains(x, y)) {
                ...
                phaseMoveMeeple = true;
                phaseSelectWhatToDo = false;
            }
